...
test("prompt includes actionable guardrails", () => {
  ...
  assert.match(prompt, /Obtain explicit user authority before deleting data, running migrations, deploying to production/);
  assert.match(prompt, /Ask when the target or action is unresolved/);
  assert.match(prompt, /check that existing approval covers them/);
  assert.match(prompt, /Never infer approval from silence/);
...
test("audit report and prompt support Korean localization", () => {
  ...
  assert.match(prompt, /프로덕션 배포/);
  assert.match(prompt, /기존 승인 범위에 포함되는지 확인하세요/);
  assert.match(prompt, /침묵에서 승인을 추론하거나/);
});
