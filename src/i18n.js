...
const MESSAGES = {
  en: {
    ...
    "finding.notGit.recommendation": "Run `git init` first, or use Vibe-Guard only in a backup-friendly experiment project.",
    "finding.missingPolicy.message": "VIBEGUARD.md is missing.",
    "finding.missingPolicy.recommendation": "Run `vibe-guard init` to create the project safety policy.",
    "finding.missingGitignore.message": ".gitignore is missing, so local secret files may be committed.",
    "finding.missingGitignore.recommendation": "Run `vibe-guard audit --fix` to create safe env ignore rules.",
    "finding.envProtection.message": ".gitignore does not fully protect local env files.",
    "finding.envProtection.recommendation": "Run `vibe-guard audit --fix` to add `.env`, `.env.*`, and `!.env.example` rules.",
    "finding.missingEnvExample.message": ".env.example is missing, so required env variable names are hard to share safely.",
    "finding.missingEnvExample.recommendation": "Run `vibe-guard audit --fix` to create a value-free example env file.",
    ...
    "finding.paidDependency.recommendation": "Confirm budget limits, request limits, and separation between test and production keys.",
    "finding.largeFile.message": "File has {lineCount} lines, which is too large for low-risk AI editing.",
    ...
    "finding.secretAssignment.recommendation": "Run `vibe-guard audit --fix` to move the value into an ignored env file and read it from the environment.",
    ...
    "report.title": "[Vibe-Guard Audit Report]",
    ...
    "report.next.fixable": "Next: {count} item(s) can be handled automatically with `vibe-guard audit . --fix`.",
    ...
    "fixes.createdAgents": "Created AGENTS.md Vibe-Guard instructions.",
    "fixes.updatedAgents": "Updated AGENTS.md Vibe-Guard instructions.",
    "fixes.addedAgents": "Added Vibe-Guard instructions to AGENTS.md.",
    ...
    "prompt.title": "# Vibe-Guard Safe Coding Prompt",
    ...
    "prompt.fixable": "Fixable: run `vibe-guard audit . --fix` or apply the same safe remediation directly.",
    ...
    "prompt.rule3": "Ignore `.env`, `.env.*`, and `.env.vibeguard.local`; keep only variable names in `.env.example`.",
  ...
  ko: {
    ...
    "finding.notGit.recommendation": "`git init`을 먼저 실행하거나, 백업 가능한 실험 프로젝트에서만 Vibe-Guard를 사용하세요.",
    "finding.missingPolicy.message": "VIBEGUARD.md가 없습니다.",
    "finding.missingPolicy.recommendation": "`vibe-guard init`으로 프로젝트 안전 정책을 생성하세요.",
    "finding.missingGitignore.message": ".gitignore가 없어 로컬 비밀 파일이 커밋될 수 있습니다.",
    "finding.missingGitignore.recommendation": "`vibe-guard audit --fix`로 안전한 env ignore 규칙을 생성하세요.",
    "finding.envProtection.message": ".gitignore가 로컬 env 파일을 충분히 보호하지 않습니다.",
    "finding.envProtection.recommendation": "`vibe-guard audit --fix`로 `.env`, `.env.*`, `!.env.example` 규칙을 추가하세요.",
    "finding.missingEnvExample.message": ".env.example이 없어 필요한 환경변수 이름을 안전하게 공유하기 어렵습니다.",
    "finding.missingEnvExample.recommendation": "`vibe-guard audit --fix`로 값 없는 예시 env 파일을 생성하세요.",
    ...
    "finding.paidDependency.recommendation": "예산 제한, 요청 횟수 제한, 테스트 키와 운영 키 분리를 확인하세요.",
    "finding.largeFile.message": "파일이 {lineCount}줄입니다. 저위험 AI 수정에는 너무 큽니다.",
    ...
    "finding.secretAssignment.recommendation": "`vibe-guard audit --fix`로 값을 ignored env 파일로 옮기고 코드에서는 환경변수로 읽게 바꾸세요.",
    ...
    "report.title": "[Vibe-Guard 점검 리포트]",
    "report.project": "프로젝트",
    ...
    "report.next.fixable": "다음: {count}개 항목은 `vibe-guard audit . --fix`로 자동 처리할 수 있습니다.",
    ...
    "fixes.createdAgents": "AGENTS.md Vibe-Guard 지침을 생성했습니다.",
    "fixes.updatedAgents": "AGENTS.md Vibe-Guard 지침을 업데이트했습니다.",
    "fixes.addedAgents": "AGENTS.md에 Vibe-Guard 지침을 추가했습니다.",
    "fixes.movedSecrets": "비밀값 {count}개를 .env.vibeguard.local로 옮겼습니다.",
    "fixes.replacedSecrets": "하드코딩된 비밀값 읽기를 교체했습니다: {files}.",

    "prompt.title": "# Vibe-Guard 안전 코딩 프롬프트",
    ...
    "prompt.fixable": "자동 수정 가능: `vibe-guard audit . --fix`를 실행하거나 동일한 안전 조치를 직접 적용하세요.",
    ...
    "prompt.rule3": "`.env`, `.env.*`, `.env.vibeguard.local`은 ignore하고, `.env.example`에는 변수 이름만 남기세요.",
