...
const MESSAGES = {
  en: {
    ...
    "finding.paidDependency.recommendation": "This is a warning, not a block. Many such services are free below a usage threshold, so check where that threshold sits for your traffic, along with budget limits, request limits, and separation between test and production keys. After review, acknowledge exact package names under `cost.acknowledgedPaidDependencies` in `.vibeguard.json`, ideally with a `reason` recording the threshold.",
    "finding.acknowledgedPaidDependency.message": "Reviewed paid or quota-based service dependency acknowledged: {dependencies}",
    "finding.acknowledgedPaidDependency.recommendation": "Acknowledgement records review only. Keep provider budgets, quotas, rate limits, and test/production separation in place.",
    "finding.paidDependencyWithoutReason.message": "Acknowledged without a recorded reason: {dependencies}",
    "finding.paidDependencyWithoutReason.recommendation": "Acknowledgement still applies. To keep the decision reviewable later, record the basis as an object in `cost.acknowledgedPaidDependencies`, for example `{ \"name\": \"resend\", \"reason\": \"free tier, 3k emails/month\", \"reviewedAt\": \"2026-07-19\" }`.",
    ...
    "prompt.rule3": "Ignore runtime env files such as `.env`, `.env.local`, and `.env.vibeguard.local`; keep env templates such as `.env.example` and `.env.sample` value-free.",
    "prompt.rule4": "Do not delete databases, run migrations, deploy to production, or increase paid API/model usage without user approval. For every real external production deployment, and any deployment whose target is unknown, immediately before execution state the exact target and action and wait for fresh user confirmation. Never infer, reuse, or bypass approval from earlier wording such as \"deploy it\" or \"handle it yourself\".",
  ...
  ko: {
    ...
    "finding.paidDependency.recommendation": "차단이 아니라 경고입니다. 이런 서비스는 일정 사용량 아래에서는 무료인 경우가 많으니, 현재 트래픽 기준으로 그 기준선이 어디인지 함께 확인하세요. 예산 제한, 요청 횟수 제한, 테스트 키와 운영 키 분리도 같이 봅니다. 검토 후 `.vibeguard.json`의 `cost.acknowledgedPaidDependencies`에 정확한 패키지명을 기록하고, 가능하면 `reason`에 그 기준선을 남기세요.",
    "finding.acknowledgedPaidDependency.message": "검토 완료한 유료 또는 쿼터 기반 서비스 의존성: {dependencies}",
    "finding.acknowledgedPaidDependency.recommendation": "확인 처리는 검토 사실만 기록합니다. 공급자 예산, 쿼터, 요청 제한, 테스트/운영 환경 분리는 계속 유지하세요.",
    "finding.paidDependencyWithoutReason.message": "근거 없이 확인 처리된 의존성: {dependencies}",
    "finding.paidDependencyWithoutReason.recommendation": "확인 처리는 그대로 유효합니다. 나중에 다시 판단할 수 있도록 `cost.acknowledgedPaidDependencies`에 객체로 근거를 남기세요. 예: `{ \"name\": \"resend\", \"reason\": \"무료 티어, 월 3천 건\", \"reviewedAt\": \"2026-07-19\" }`.",
    ...
    "prompt.rule3": "`.env`, `.env.local`, `.env.vibeguard.local` 같은 런타임 env 파일은 ignore하고, `.env.example`, `.env.sample` 같은 env 템플릿에는 변수 이름과 placeholder만 남기세요.",
    "prompt.rule4": "사용자 승인 없이 DB 삭제, 마이그레이션, 운영 배포, 유료 API/model 사용 증가를 실행하지 마세요. 실제 외부 운영 배포와 대상이 불명확한 모든 배포는 실행 직전에 정확한 대상과 작업을 명시하고, 사용자의 새 승인을 기다리세요. 이전의 \"배포해\" 또는 \"알아서 처리해\" 같은 표현으로 승인을 추론·재사용·우회하지 마세요.",
